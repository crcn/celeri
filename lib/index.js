...
exports.parse = function(args, ignoreEmit) {
    ...
    while(args.length) {
        ...
        if(arg.substr(0,2) != '--') {
            command.push(encodeURIComponent(arg));
        } else {
            args.unshift(arg);
            break;
        }
    ...
    haba.next(function() {
        ...
        if(!self.emit(command.join(' '), data)) {
            if(command.join('').length) console.error('command "%s" does not exist', command.join(' ').bold);
            self.emit('help');
        }  
