...
exports.plugin = function(cli) {
	function normalizeOptions(ops) {
		...
		for(var i = 0; i < numCols; i++) { 
			...
			actualWidth = Math.max(column.minWidth, ops.fixed ? column.width : percWidth),
	...
	function addLines(colLines, value, column) {
		...
		var newLines = splitLines(value);

		
		for(var i = 0, n = newLines.length; i < n; i++) {
			addLine(colLines, newLines[i], column);
		} 
	}

	function addLine(colLines, value, column) {

		//trim whitespace off the ends. line breaks = whitespace
		var buffer = value.replace(/^\s+|\s+$/g,'');
		...
		switch(column.align) {
			...
			default: 
			if(!column.last) buffer = cli.utils.padRight(buffer, padding, ' ');
			break;
		}
	...
	function getRowLineTable(item, ops) {
		...
		for(var j = 0, jn = cols; j < jn; j++) {
			...
			var explicitLines = splitLines(value);


			for(var k = 0, kn = explicitLines.length; k < kn; k++) {


				var explicitLineValue = explicitLines[k];

				if(explicitLineValue.length > actualWidth) {

					if(ops.ellipsis) {
						var newBuffer = explicitLineValue.substr(0, actualWidth-3) + cli.utils.repeat('.', Math.min(actualWidth, 3));
						...
						for(var k = actualWidth; k < explicitLineValue.length + actualWidth; k += actualWidth) {

							addLines(colLines, explicitLineValue.substr(start, actualWidth), column);
							start  = k;
	                    }
					...
					addLines(colLines, explicitLineValue, column);
	...
	cli.drawTable = cli.table = function(source, ops) {
